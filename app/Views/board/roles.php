      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            ...
            <br>
              <p>
                <a href="<?=base_url()?>roles/insert" class="btn btn-app">
                  <span class="badge bg-success">Nuevo</span>
                  <i class="fas fa-barcode"></i> Roles
                </a>
                <a href="<?=base_url()?>roles/recovery" class="btn btn-app bg-danger" > 
    ...
    <!-- Main content -->
      ...
      <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr>
                    <td><?= esc($news_item['idroles']); ?></td>
                    <td><?= esc($news_item['name']); ?></td>
                    <td>
                        <a href="<?=base_url()?>roles/permiso?id=<?= esc($news_item['idroles']); ?>">
                            <button type="button" class="btn btn-warning"> 
                            <i class="fas fa-sitemap"></i> ROLES</button>
                        </a>
                    </td>
                    <td>
                      <div class="btn-group btn-group-sm">
                        <a href="<?=base_url()?>roles/update?id=<?= esc($news_item['idroles']); ?>"><button type="button" class="btn btn-warning"> <i class="fas fa-edit"></i> Editar</button></a>
                        <button type="button" onclick="mydelete('<?= esc($news_item['idroles']); ?>','<?= esc($news_item['name']); ?>')" class="btn btn-danger"><i class="fas fa-trash-alt"></i> Eliminar</button>
                      </div>
                    </td>
                  </tr>
                  <?php  endforeach; ?>
...
<script>
  $(function () {
    ...
    swalWithBootstrapButtons.fire({
      ...
      if (id == 1 ) {
        swalWithBootstrapButtons.fire(
          'No se puede eliminara al administrador!',
          'Su archivo no ha sido eliminado.',
          'success'
        )
      }else
      if (result.isConfirmed) {
        location.href='<?=base_url()?>roles/delete?id='+id
        // swalWithBootstrapButtons.fire(
        //   'Eliminado!',
